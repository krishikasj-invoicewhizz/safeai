...
async function sendSOSAlerts(pos) {
  ...
  try {
    ...
    // In a real app you'd trigger Firebase Cloud Functions / SMS / email here.
    // For MVP we log and show a toast.
    console.log("SOS Alert would be sent to:", contacts.map(c => c.name).join(", ") || "No contacts saved");
    console.log("Location link:", locStr);

    if (contacts.length === 0) {
      toast("⚠️ No emergency contacts saved! Add contacts now.");
    } else {
      toast(`🚨 SOS sent to ${contacts.length} contact(s)!`);
    }
  } catch (e) {
